//charles enright 5/19/16
//CMPS 335
//Exam framework.
// Test function begins on line 141. Helper function begins on line 224.
...
vector<SBSI*>* format_data(double[][5]);
void test(vector<SBSI*> *);
...
void test(vector<SBSI*> * data, Flt tolerance = 0.005){
    Flt y_actual;
    cout << "H\t\tWC\t\tBSA\t\tSBSI\t\t"
        //<< "Expected\t\t"
        << "Function\n"
        <<  "---\t\t---\t\t-----\t\t---\t\t-----"
        << endl;
        for(vector<SBSI*>::iterator i = data->begin();
        i != data->end();
        i++
    ){
        y_actual = bodyShapeIndex(*i);
        cout << (*i)->h << "\t\t";
        cout << (*i)->wc << "\t\t";
        cout << (*i)->bsa << "\t\t";
        cout << (*i)->vtc << "\t\t";
        //cout << (*i)->expected << "\t\t";
        cout << y_actual << "\t";
        if(abs(y_actual - (*i)->expected) > tolerance)
...
vector<SBSI*>* format_data(double data[][5])
{
    static vector<SBSI*> output;
